(this.workbox = this.workbox || {}),
  (this.workbox.broadcastUpdate = (function(e, t) {
    ...
    const s = (e, t, s) => {
      return (
        ...
        s.every(s => {
          ...
          const a = e.headers.get(s) === t.headers.get(s);
          return n && a;
        })
      );
    };
    const n = 'workbox';
    const a = 1e4;
    const i = ['content-length', 'etag', 'last-modified'];
    const o = async ({ channel: e, cacheName: t, url: s }) => {
      const n = {
        type: 'CACHE_UPDATED',
    ...
    class c {
      ...
      notifyIfUpdated({ oldResponse: e, newResponse: t, url: n, cacheName: a, event: i }) {
        if (!s(e, t, this.t)) {
          const e = (async () => {
            i && i.request && i.request.mode === 'navigate' && (await this.h(i)),
      ...
      u() {
        return (
          'BroadcastChannel' in self && !this.p && (this.p = new BroadcastChannel(this.s)), this.p
        );
      }
      ...
      o() {
        (this.m = new Map()),
          self.addEventListener('message', e => {
            if (
              e.data.type === 'WINDOW_READY' &&
              e.data.meta === 'workbox-window' &&
              this.m.size > 0
            ) {
