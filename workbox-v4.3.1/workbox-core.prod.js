(this.workbox = this.workbox || {}),
  (this.workbox.core = (function(e) {
    ...
    const a = e => [r.prefix, e, r.suffix].filter(e => e.length > 0).join('-');
    const i = {
      updateDetails: e => {
    ...
    const u = 'cacheKeyWillBeUsed';
    const h = 'cacheWillUpdate';
    const f = 'cachedResponseWillBeUsed';
    const w = 'fetchDidFail';
    const g = 'fetchDidSucceed';
    const d = 'requestWillFetch';
    const p = (e, t) => e.filter(e => t in e);
    const y = async ({ cacheName: e, request: t, event: n, matchOptions: s, plugins: r = [] }) => {
      const a = await caches.open(e);
      const i = await q({ plugins: r, request: t, mode: 'read' });
    ...
    const m = async ({ request: e, response: t, event: n, plugins: s }) => {
      let r = t;
      let a = !1;
      for (const t of s)
        if (h in t && ((a = !0), !(r = await t[h].call(t, { request: e, response: r, event: n }))))
          break;
      return a || (r = r.status === 200 ? r : null), r || null;
    };
    const q = async ({ request: e, mode: t, plugins: n }) => {
      const s = p(n, u);
      let r = e;
      for (const e of s)
        typeof (r = await e[u].call(e, { mode: t, request: r })) === 'string' &&
          (r = new Request(r));
      return r;
    };
    const v = {
      put: async ({
        ...
        const h = await m({ event: r, plugins: a, response: s, request: u });
        if (!h) return;
        const f = await caches.open(e);
        const w = p(a, l);
        const g = w.length > 0 ? await y({ cacheName: e, matchOptions: i, request: u }) : null;
        try {
          await f.put(u, h);
        } catch (e) {
          throw (e.name === 'QuotaExceededError' && (await o()), e);
        }
        for (const t of w)
          await t[l].call(t, {
            cacheName: e,
    ...
    const D = {
      fetch: async ({ request: e, fetchOptions: t, event: s, plugins: r = [] }) => {
        ...
        const a = p(r, w);
        const i = a.length > 0 ? e.clone() : null;
        try {
          for (const t of r) d in t && (e = await t[d].call(t, { request: e.clone(), event: s }));
        } catch (e) {
        ...
        try {
          let n;
          n = e.mode === 'navigate' ? await fetch(e) : await fetch(e, t);
          for (const e of r)
